...
system. Statistical thermodynamic is a tool that we can use to bring together
...
Everyday chemical systems contains a huge number of atoms, of the order of
...
around \SI{e10}{} atoms. This is even worst if we consider the number of atoms
...
At these scales, the surface effects are not negligible: the surface energy
becomes an important part of the overall energy. The typical way to remove these
surface effects while keeping the number of simulated atoms low is to use
\emph{periodic boundary conditions}. The atoms are placed in a rhombohedral
simulation box called the \emph{unit cell}, which is replicated \emph{ad
infinitum} in all directions, as illustrated in figure~\ref{fig:pbc}. The copies
of the initial box are called images of this box.
...
\begin{figure}[b]
    ...
    system. Molecules from the central cell are \emph{repeated} in all
    directions.}
    \label{fig:pbc}
\end{figure}
...
boxes, including with molecules images of itself. The effectively infinite
...
periodicity to the system, more so as the central simulation box is smaller. For
the study of crystalline materials, one has to ensure that the periodicity of
the simulation box matches the periodicity of the crystal primitive cell. This
periodicity also makes it harder to study defects and other rare features of the
system. If we explicitly add a defect to the system, it will be replicated over
all the images, artificially increasing the number of defects per unit of
volume.
...
order to describe electrostatic interaction properly in presence of periodic
boundary conditions, we need to use other methods such as the Ewald summation,
which I will present in more details in chapter~\ref{sec:electrostatic}.
...
For a orthorhombic simulation box, the smallest box side length must be at least
