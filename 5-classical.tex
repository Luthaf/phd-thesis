...
in \ZIF8, from static, dynamic and thermodynamic points of view. In the second
...
that \ce{MgCl2} ions can enter a pure-silica ferrierite\cite{Arletti2016}. All
...
During my PhD, I have studied high-pressure electrolyte intrusion in \ZIF8.
\ZIF8 is hydrophobic\cite{AOrtiz2014} and presents interesting behavior upon
intrusion. While osmotic pressure effects do not depend on the chemical nature
of the ions but only of their concentration, \ZIF8 shows changes from one
energetic behavior to another when the ion nature changes while keeping
concentration constant\cite{Ortiz2014}. \ZIF8 behavior can also be tuned
...
electrolytes intrusion in \ZIF8 are still unknown. In this work, I used
...
properties of \ZIF8 and the impact of liquid intrusion on them. I also look at
the energetic behavior of intrusion, and the thermodynamics of the ions' entry
in \ZIF8. This work is published in \citejournal{Fraux2019-2}\cite{Fraux2019-2}.
...
\citeauthor{Zheng2012}\cite{Zheng2012} for the description of the \ZIF8
...
confined in \ZIF8, I needed to know how many molecules would fit in the \ZIF8
pores. For that, I ran a constant pressure simulation with a reservoir of pure
water outside an empty $2\times2\times3$ \ZIF8 super-cell at \SI{0.5}{GPa} for
\SI{10}{ns} and counted 75 water molecules by unit cell that entered \ZIF8. I
...
The first thing we can see here is that, for all species (\ce{Li+}, \ce{Cl-},
...
solvation state: 4 \ce{H2O} around each \ce{Li+}, and 6 \ce{H2O} around each
\ce{Cl-}. But as the concentration increases, there are fewer available water
...
molecules are geometrically constrained by the presence of the \ZIF8 framework,
...
So far, I have studied the properties of the confined liquid inside the \ZIF8
...
under intrusion. I used the distribution of the \ce{Zn-Zn-Zn-CH3} dihedral swing
...
fluctuations of the order of 10°. The distributions for \ZIF8 containing the
...
\SI{77}{K} in \ZIF8; where the equilibrium value goes from 10° to 25° as the
...
\begin{figure}[ht]
    ...
    \caption{Changes in volume of bulk liquid (top) and \ZIF8 with confined electrolyte
...
response is almost linear. Moreover, confining an electrolyte in the \ZIF8
...
Adding water to the pores of \ZIF8 only changes the bulk modulus by a moderate
amount (10\%), meaning that most of the stiffness comes from the \ZIF8 framework
...
modeled an explicit water/\ZIF8 interface, depicted in
...
is not even clear what fraction of the larger cations (\ce{K+}) actually can
...
\citejournal{Scalfi2018}\cite{Scalfi2018}, and is described in this section.
...
\ce{Al2SiO3(OH)4}, that was first discovered in volcanic ashes in
...
folding, with a variable number $n$ of \ce{Al2O3SiOH(OH)3} units (called
...
bonds (\ce{O-H} stretching); and a \ce{M-O-H} harmonic bending potential.
...
% \end{figure}
%
% Secondly, we focus on the description of the internal and outer surface groups. We
...
\begin{figure}[ht]
    ...
    atoms (Si in green, O in red, H in black) and water molecules (\ce{O_w} in
    blue and \ce{H_w} in orange). Silicon rings are indicated by dashed lines.}
...
indicated by the well-defined positions for \ce{H_{int}} atoms, which are
...
\ce{O_{int}} atom. This conformation allows the formation a relatively weak
hydrogen bond, because the \ce{O_{int}-H_{int}$\cdots$O_{int}} distance is
