...
Python, but developers are able to optimize it further. Domino uses the \cxx11
...
contributions should be skipped or scaled down as necessary. For the long-range
term, we only want to remove interaction between atoms in the same molecule and
in the same simulation box image. Instead of changing the sum over the $\vec k$
vectors, we explicitly remove the corresponding contribution --- leading to the
molecular correction term in equation~\ref{eq:ewald}.
...
\end{array} \]
\vskip-3em
Here, $\mathcal{I}\kern-0.2ex m$ is the imaginary part of a complex number; and
the self interaction and charge correction do not contribute to the forces or
stress.
