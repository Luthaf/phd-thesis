...
$E_\text{kin}$, potential energy $E_\text{pot}$ and internal energy $U$, and
noting $W$ the work received by a system during a transformation and $Q$ the
heat transfer during the same transformation, the first law of thermodynamics
can be written as
\[\Delta E_\text{tot} = \Delta E_\text{kin} + \Delta E_\text{pot} + \Delta U = W + Q.\]
If now we consider a system at rest, $E_\text{kin}$ and $E_\text{pot}$ are
...
considering infinitesimal transformations and changes in $U$, $W$, and $Q$:
\[\d U = \delta Q + \delta W.\]
...
each chemical specie in the system $\{N_i\}$:
\[\d U = \delta Q + \delta W - \sum_i \mu_i \d N_i.\label{eq:first-principle}\]
...
-\d (p_\text{ext} V)$. We can then define the enthalpy $H$ as $H = U + PV$ ;
which allow us to rewrite equation~\ref{eq:first-principle} as
...
\vskip\baselineskip
Using the relations~\ref{eq:first-principle} and~\ref{eq:clausius}; and
...
micro-canonical ensemble, we start with equation~\ref{eq:thermo-potentials}:
\[\d \Phi = \d U + P \d V - \sum_i \mu_i \d N_i - T \d S.\]
$\d U$ and $\d V$ will be zero as the energy and volume is constant. Additionaly
as the system composition is fixed, $\d N_i = 0$. This gives
\[\d \Phi = - T \d S \leq 0.\]
The thermodynamic potential for the $NVE$ ensemble is the negentropy $-TS$,
which will be minimal (and the entropy will be maximal) at equilibrium.
...
equation~\ref{eq:thermo-potentials} becomes
...
adsorption. In this case, equation~\ref{eq:thermo-potentials} becomes
...
1998\cite{Escobedo1998}. Again, starting from equation~\ref{eq:thermo-potentials}
\[\d \Phi = \d U - \d (PV) - \d(TS) - V\d P + S\d T - \sum_i \mu_i \d N_i.\]
...
\[\Omega = U - TS + PV - \sum_i \mu_i N_i \equiv F + PV - \sum_i \mu_i N_i \equiv G - \sum_i \mu_i N_i\]
...
\begin{table}[htp]
    ...
    \begin{tabularx}{0.8\textwidth}{c C c}
        Thermodynamic potential & Fixed quantities                     & Ensemble \\ \hline
                $-TS$           & $N$, $V$, $E$                        & Micro-canonical \\
                $F$             & $N$, $V$, $T$                        & Canonical \\
                $G$             & $N$, $P$, $T$                        & Isobaric-Isotherm \\
                $\Psi$          & $\mu_i$, $V$, $T$                    & Grand-Canonical \\
                $\Omega$        & $N_\text{host}$, $\mu_i$, $P$, $T$   & Osmotic \\
    \end{tabularx}
...
\fbox{introduce direct thermodynamics calculations}
...
interpreted by different authors, \citeauthor{Myers1965} originally meant it to
...
amount of published data from direct experimental measurements of co-adsorption
...
\begin{figure}[htp]
    \centering
    \raisebox{-0.5\height}{\includegraphics[width=0.35\textwidth]{figures/cu-dhbc-structure}}
    \hfill
...
the isotherms (equation~\ref{eq:iast}).
...
\begin{figure}[htp]
    \centering
    \raisebox{-0.5\height}{\includegraphics[width=0.35\textwidth]{figures/rpm3-zn-structure}}
    \hfill
